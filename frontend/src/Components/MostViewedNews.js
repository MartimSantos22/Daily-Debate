...
function MostViewedNews() {
  ...
  const outrasNoticias = noticias.slice(1, 6); // mostra até 5 outras mais vistas
 ...
 return (
    <div className="FaixaAzulContainer">
      ...
      >
        ...
        <div className="ContainerImagemTitulo" onClick={() => handleNewsClick(noticiaPrincipal)}>
          <img
            className="Imagem-MostViewed"
            src={noticiaPrincipal.imagem}
            alt={noticiaPrincipal.titulo}
          />
          <h2 className="TituloMostViewed">
  ...
  {outrasNoticias.map(noticia => (
    <li key={noticia.id} onClick={() => handleNewsClick(noticia)} className="ItemNoticia">
      {noticia.titulo}
    </li>
  ))}
</ul>
